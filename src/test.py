...
HUMAN_SPAWN_XYZ: tuple[float, float, float] = (-2.78, -0.93, 0.0)
...
HUMAN_BLOCKER_PRIM_PATH = f"{CHARACTER_CONTROL_PRIM_PATH}/CollisionCylinder"
HUMAN_BLOCKER_RADIUS = 0.38
HUMAN_BLOCKER_HEIGHT = 1.75
...
class RaycastGuardWaypointController:
    """Waypoint controller that pauses while a configured blocker is in the forward ray."""
    ...
    def __init__(
        ...
        self.blocker_prim_paths = tuple(blocker_prim_paths)
    ...
    def _raycast_forward(self, x: float, y: float, dir_x: float, dir_y: float, max_dist: float) -> tuple[float, str] | None:
        ...
        for ray_height in RAYCAST_HEIGHTS:
            ...
            if hit_path == "":
                continue
            if not any(hit_path == prim_path or hit_path.startswith(f"{prim_path}/") for prim_path in self.blocker_prim_paths):
                continue
    ...
    def update(self, dt: float) -> None:
        ...
        robot_xyz = get_robot_root_xyz(self.robot)

        if RAYCAST_GUARD_ENABLED:
            hit = self._raycast_forward(robot_xyz[0], robot_xyz[1], dir_x, dir_y, RAYCAST_LOOKAHEAD)
            if hit is not None:
...
def spawn_human_blocker(stage: Usd.Stage) -> SingleRigidPrim:
    blocker = UsdGeom.Cylinder.Define(stage, HUMAN_BLOCKER_PRIM_PATH)
    blocker.CreateRadiusAttr(HUMAN_BLOCKER_RADIUS)
    blocker.CreateHeightAttr(HUMAN_BLOCKER_HEIGHT)
    blocker.CreateAxisAttr("Z")
    blocker.CreateDisplayColorAttr().Set([Gf.Vec3f(0.15, 0.75, 0.25)])

    center_xyz = (
        HUMAN_SPAWN_XYZ[0],
        HUMAN_SPAWN_XYZ[1],
        HUMAN_SPAWN_XYZ[2] + 0.5 * HUMAN_BLOCKER_HEIGHT,
    )
    UsdGeom.XformCommonAPI(blocker.GetPrim()).SetTranslate(Gf.Vec3d(*center_xyz))
    UsdGeom.Imageable(blocker.GetPrim()).MakeInvisible()

    collision_api = (
        UsdPhysics.CollisionAPI(blocker.GetPrim())
        if blocker.GetPrim().HasAPI(UsdPhysics.CollisionAPI)
        else UsdPhysics.CollisionAPI.Apply(blocker.GetPrim())
    )
    collision_api.CreateCollisionEnabledAttr(True)

    physx_collision_api = (
        PhysxSchema.PhysxCollisionAPI(blocker.GetPrim())
        if blocker.GetPrim().HasAPI(PhysxSchema.PhysxCollisionAPI)
        else PhysxSchema.PhysxCollisionAPI.Apply(blocker.GetPrim())
    )
    physx_collision_api.CreateContactOffsetAttr(0.02)
    physx_collision_api.CreateRestOffsetAttr(0.0)

    rigid_body_api = (
        UsdPhysics.RigidBodyAPI(blocker.GetPrim())
        if blocker.GetPrim().HasAPI(UsdPhysics.RigidBodyAPI)
        else UsdPhysics.RigidBodyAPI.Apply(blocker.GetPrim())
    )
    rigid_body_api.CreateRigidBodyEnabledAttr(True)
    rigid_body_api.CreateKinematicEnabledAttr(True)

    physx_rigid_body_api = (
        PhysxSchema.PhysxRigidBodyAPI(blocker.GetPrim())
        if blocker.GetPrim().HasAPI(PhysxSchema.PhysxRigidBodyAPI)
        else PhysxSchema.PhysxRigidBodyAPI.Apply(blocker.GetPrim())
    )
    physx_rigid_body_api.GetDisableGravityAttr().Set(True)

    blocker_body = SingleRigidPrim(HUMAN_BLOCKER_PRIM_PATH, name="human_blocker", reset_xform_properties=False)
    print(
        f"[Human] spawned blocker at {center_xyz} "
        f"(radius={HUMAN_BLOCKER_RADIUS:.2f}, height={HUMAN_BLOCKER_HEIGHT:.2f})."
    )
    return blocker_body


def sync_human_blocker_pose(human_blocker: SingleRigidPrim, human_xyz: tuple[float, float, float]) -> None:
    center_xyz = (
        human_xyz[0],
        human_xyz[1],
        human_xyz[2] + 0.5 * HUMAN_BLOCKER_HEIGHT,
    )
    _, orientation = human_blocker.get_world_pose()
    zero_velocity = human_blocker._backend_utils.create_zeros_tensor(
        shape=[1, 6],
        dtype="float32",
        device=human_blocker._device,
    )
    human_blocker.set_world_pose(position=np.array(center_xyz, dtype=np.float32), orientation=orientation)
    human_blocker._rigid_prim_view.set_velocities(velocities=zero_velocity)
    UsdGeom.XformCommonAPI(human_blocker.prim).SetTranslate(Gf.Vec3d(*center_xyz))
...
class TaskRuntime:
    ...
    human_blocker: SingleRigidPrim | None = None
...
def step_runtime(runtime: TaskRuntime) -> None:
    ...
    if runtime.human_agent is not None and runtime.human_blocker is not None:
        sync_human_blocker_pose(runtime.human_blocker, runtime.human_agent.get_current_xyz())
...
def main():
    ...
    human_blocker = spawn_human_blocker(stage)
    ...
    controller = RaycastGuardWaypointController(
        robot=robot,
        waypoints_xyz=[],
        ...
        yaw_tolerance_rad=math.radians(args.yaw_tolerance_deg),
        blocker_prim_paths=[HUMAN_BLOCKER_PRIM_PATH],
    )
    controller.initialize()
    human_blocker.initialize()
    if human_agent is not None:
        sync_human_blocker_pose(human_blocker, human_agent.get_current_xyz())
    grabber = ProximityGrabber(
        stage=stage,
        robot=robot,
    ...
    if RAYCAST_GUARD_ENABLED:
        print(
            f"[Raycast] enabled for blocker {HUMAN_BLOCKER_PRIM_PATH} "
            f"(lookahead={RAYCAST_LOOKAHEAD:.2f}, heights={RAYCAST_HEIGHTS})"
        )
